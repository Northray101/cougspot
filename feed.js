/* ════════════════════════════════════════════
   CougSpot — Feed Page
   Standalone script for feed.html
   ════════════════════════════════════════════ */
...
function isSpam(text) { return SPAM_PATTERNS.some(r => r.test(text)); }
async function flagContent(type, id, userId, username, text, flagTypes) {
  for (const ft of flagTypes) {
    await sb.from('flags').insert({
      content_type: type, content_id: id, user_id: userId,
      username, content_text: text.slice(0, 400), flag_type: ft, auto_flagged: true,
    });
  }
}
...
/* ════════════════════════════════════════════
   SCREEN ROUTER (needed for social.js overlays)
════════════════════════════════════════════ */
...
function enterFeed() {
  ...
  ['nav-avatar-feed', 'compose-avatar'].forEach(id => {
...
async function submitPost() {
  ...
  const { data: inserted, error } = await sb.from('posts').insert({
    content, tag: currentPostTag, user_id: currentUser.id,
    username, is_anon: postAnonMode, source: 'user', hidden: false, spam_score: 0,
  }).select().single();
...
async function loadComments(postId) {
  ...
  const { data, error } = await sb.from('comments').select('*').eq('post_id', postId).is('parent_id', null).eq('hidden', false).order('created_at', { ascending: true });
  if (error) { section.innerHTML = `<div style="padding:12px;font-size:12px;color:var(--text-faint)">Could not load comments.</div>`; return; }

  const { data: replies } = await sb.from('comments').select('*').eq('post_id', postId).not('parent_id', 'is', null).eq('hidden', false).order('created_at', { ascending: true });
  const replyMap = {};
  (replies || []).forEach(r => {
  ...
  section.innerHTML = `<div class="comments-list">${commentHTML || '<div class="comment-empty">No comments yet.</div>'}</div>${renderCommentBox(postId, null)}`;
}
...
async function submitComment(postId, parentId, anonBtnId, inputId) {
  ...
  const { data: inserted, error } = await sb.from('comments').insert({
    ...
    user_id: currentUser.id, username, content, is_anon: isAnon, hidden: false,
  }).select().single();
...
function subscribeAnnouncements() {
  if (announceChannel) sb.removeChannel(announceChannel);
  announceChannel = sb.channel('announcements-rt-feed')
...
function subscribeRealtime() {
  if (postsChannel) sb.removeChannel(postsChannel);
  postsChannel = sb.channel('posts-rt-feed')
